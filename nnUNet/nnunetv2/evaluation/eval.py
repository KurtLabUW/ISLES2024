...
def get_LesionWiseResults(pred_file, gt_file, challenge_name, output=None):
    ...
    if challenge_name == 'BraTS-GLI' or challenge_name == 'BraTS-GoAT':
        dilation_factor = 3
    ...
    for l in range(len(label_values)):
        ...
        metric_df.hd95_lesionwise.replace(np.inf, 374, inplace=True)

        final_lesionwise_metrics_df = pd.concat((final_lesionwise_metrics_df, metric_df)) # Adjusted by Ethan
